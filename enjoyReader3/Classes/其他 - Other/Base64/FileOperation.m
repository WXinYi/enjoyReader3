...
/**
 *  新创建一个名为FileName 的文件夹在  某个 目录下
 *
 *  @param fileName      新建的文件夹的名称
 *  @param directoryName 上一级的目录
 */
+(void)createFileNamed:(NSString *)fileName inDirectory:(NSString *)directoryName{
...
//将数组转json 加密 存到制定路径 文件名字
/**
 *  将数组转json 加密 存到制定路径 文件名字
 *
 *  @param fileName      文件名 (注意需要传入文件后缀 如：@“周杰伦.avi”)
 *  @param directoryName 存放在哪个目录
 *  @param contentArr    要存储的数组
 */
+(void)createFileNamed:(NSString *)fileName inDirectory:(NSString *)directoryName andContentArray:(NSArray *)contentArr{
    
    NSFileManager *fileManager = [NSFileManager defaultManager];
    
    NSString *txt =  [directoryName stringByAppendingPathComponent:fileName];
    
    NSString * contents = [YXJSON JSONStringWithDictionaryOrArray:contentArr];
    
    contents = [WXYClassMethodsViewController encrypt:contents];
