...
@implementation NSString (Encrypt3DESandBase64)
- (NSString *)encryptStringWithKey:(NSString*)_encryptKey
{
    ...
    NSString *string = [WXYGTMBase64 stringByEncodingData:[NSData dataWithBytes:(const void *)bufferPtr length:(NSUInteger)movedBytes]];
    free(bufferPtr);
    return string;
}
- (NSString *)decryptStringWithKey:(NSString*)_encryptKey
{
    ...
    void *  bufferPtr ;
    _encryptKey = (_encryptKey == nil)? [self get24BitKey:@"abcd12345678901234567890"]: [self get24BitKey:_encryptKey];
    NSData *encryptedStringData = [WXYGTMBase64 decodeData:[self dataUsingEncoding:NSUTF8StringEncoding]];
