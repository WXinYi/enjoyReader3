...
//计算高度的方法****************************
    ...
    NSInteger  point = suggestedSize.height;
    MyNSLog("height == %f",suggestedSize.height);
    NSUserDefaults * height = [NSUserDefaults standardUserDefaults];
    [height setInteger:point forKey:@"pageHeight"];
    [height synchronize];
//****************************************
...
- (void)showCursor{
    ...
    [self removeCursor];
    _leftCursor = [[CursorView alloc] initWithType:CursorLeft andHeight:self.font byDrawColor:[UIColor blueColor]];
    _rightCursor = [[CursorView alloc] initWithType:CursorRight andHeight:self.font byDrawColor:[UIColor blueColor]];
...
- (void)copyword:(id)sender{
    ...
    [pasteboard setString:[NSString stringWithFormat:@"%@",[self.text substringWithRange:NSMakeRange(selectedRange.location, selectedRange.length)]]];
     ...
     MyNSLog("copyString == %@",[NSString stringWithFormat:@"%@",[self.text substringWithRange:NSMakeRange(selectedRange.location, selectedRange.length)]]);
    [self resetting];
}
