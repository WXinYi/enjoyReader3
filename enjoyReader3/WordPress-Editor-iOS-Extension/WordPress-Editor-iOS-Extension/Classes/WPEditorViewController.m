...
- (instancetype)init
{
	return [self initWithMode:kWPEditorViewControllerModePreview];
}
...
- (UIBarButtonItem*)showSourceBarButton
{
    ...
    ZSSBarButtonItem *barButtonItem = [self barButtonItemWithTag:kWPEditorViewControllerElementShowSourceBarButton
													htmlProperty:@"source"
													   imageName:@"icon_format_keyboard"
														  target:self
														selector:@selector(hideKeyboard)
											  accessibilityLabel:accessibilityLabel];//showHTMLSource:
...
- (void)buildTextViews
{
    if (!self.editorView) {
        ...
        CGRect frame = CGRectMake(0.0, 0
                                  , viewWidth, CGRectGetHeight(self.view.frame));
...
- (void)setBold
{
    ...
    ChooseExtractViewController *vc = [[ChooseExtractViewController alloc] init];
